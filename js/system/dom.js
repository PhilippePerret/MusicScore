...
class DOM {
  ...
  static clone(selector, newAttrs = {}){
    const clone = document.body.querySelector(selector).cloneNode(true)
    newAttrs.id     && (clone.id = newAttrs.id)
    newAttrs.class  && (clone.className = newAttrs.class)
    return clone
  }
  constructor(domEl){
    this.obj = domEl
